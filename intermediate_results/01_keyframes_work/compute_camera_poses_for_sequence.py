...
while(True):
    ...
    #print(success)
    #print(inliers)
    # R_current and t_current represent the absolute camera pose w.r.t. world coordinates
    R_current = cv2.Rodrigues(rvec)[0].T
    t_current = -np.matmul(cv2.Rodrigues(rvec)[0].T, tvec)
    ...
    # current_pose = to_twist(R_current, t_current).reshape(6, 1)
    # current_dist = np.matmul(np.matmul(current_pose.T, pose_distance_weights), current_pose)
    # current_pose = current_pose.reshape(6,)
    # print(current_dist)
    ...
    if current_dist >= pose_distance_threshold:
        ...
        # insert new keyframe and edge into pose graph
        ...
        #pickle.dump(current_pose, open("new_pose.pkl", "wb"))
        #prev_node_id = sorted(pose_graph.nodes)[-1]
